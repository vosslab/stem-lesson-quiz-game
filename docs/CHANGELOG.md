...
**Shop preview no longer effectively equips theme (auto-revert).** Daughter eval: "I can use new themes without buying." Tap and hover handlers in [src/scene_shop.ts](../src/scene_shop.ts) called `document.body.setAttribute("data-theme", theme.id)`, and on touch devices `mouseleave` never fires, so the preview stuck until something else restored the equipped theme -- a free equip. Kept the full-page preview (it is the whole point) but added a scene-wide `preview_timer` (`PREVIEW_REVERT_MS = 2500`) that always restores the real equipped theme after 2.5s. Tapping a different card (or hovering on desktop) cancels and restarts the timer so previews cannot stack. Back button and Buy-button click both call `cancel_preview()` so the kid never sees a stale preview behind the confirmation modal or after leaving the shop. Esc-out from shop is bounded too: the timer keeps running after the scene unmounts and restores within 2.5s. Added a short `shop-card-tap` pulse animation in [src/style.css](../src/style.css) for tactile feedback on the tapped card.

**Defensive save load + reset escape hatch (GitHub Pages buttons dead).** Kid report: on Pages, lesson select/clear works but mode cards and Shop do nothing. Root cause hypothesis: stale localStorage from earlier schema versions where the v2 -> v3 migration only seeded `stats_today` sub-fields, leaving top-level required fields (`lifetime_coins`, `owned_themes`, `last_choices_by_mode`, etc.) undefined; downstream code (`save.lifetime_coins += x`, `s.last_choices_by_mode[mode_id] = count`) then NaNs / throws and silently kills click handlers. [src/persist.ts](../src/persist.ts) `read_raw` now (a) wraps `JSON.parse` in try/catch and returns `default_save()` on any parse failure, (b) rejects non-object parsed values, (c) adds `backfill_top_level()` that copies any missing top-level keys from `default_save()` into the migrated blob before adopting it (preserves the kid's coins/mastery/themes; only fills blanks). New exported `reset_save()` clears `localStorage[STORAGE_KEY]` and the in-memory cache. [src/scene_mastery.ts](../src/scene_mastery.ts) adds a muted, dashed "Reset save" button at the bottom of the mastery page (not home -- intentionally tucked away to discourage idle taps) with a two-step `window.confirm` gate and a hard `location.reload()` on confirm. Styled via new `.mastery-reset-row` / `.mastery-reset-button` selectors in [src/style.css](../src/style.css).
