...
async function goto_home(page, theme, coins) {
  await page.evaluate(
    ...
    [STORAGE_KEY, make_save(theme, coins, null)],
  );
...
async function run_home(browser, theme, vp, _label) {
...
async function run_wrong_feedback(browser, theme, vp, choice_count) {
  ...
  if (!wrong_reached) {
    console.log(
      "[matrix] WARN: wrong-state not reached for",
      theme,
      "- retrying with different button",
    );
