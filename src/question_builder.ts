...
export function pick_next_question(
	...
	if (!selected_stem) {
		// Pick from selected lessons
		// LessonId format is "L1", "L2", etc. Extract the number after "L"
		...
		// Fallback to all stems if not enough selected
		const pool = selected_stems.length > 0 ? selected_stems : bundle.all_stems;
		selected_stem = pool[Math.floor(Math.random() * pool.length)];
	}
