...
export async function flash_answer_feedback(
  ...
  const chosen_btn = scene_el.querySelector<HTMLButtonElement>(
  ...
  const choices_grid = scene_el.querySelector(".choices-grid");
  if (choices_grid) {
  ...
  if (chosen_btn) {
    if (is_correct) {
      ...
      const correct_btn = scene_el.querySelector<HTMLButtonElement>(
  ...
  if (is_correct) {
    ...
    const hint_chip = teaching_panel?.querySelector(".continue-hint") as HTMLElement | null;
